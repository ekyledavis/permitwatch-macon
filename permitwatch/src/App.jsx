...
export default function App(){
  ...
  const statuses=["All","Under Review","Pending Hearing","Approved","Denied","Decision Issued"];
  ...
  const StatusPills=()=>(
    <>{statuses.map(s=>(
      <button key={s} onClick={()=>setStatusFilter(s)} style={{background:statusFilter===s?"#4F6BFF22":"transparent",border:`1px solid ${statusFilter===s?"#4F6BFF":"#2A2E42"}`,borderRadius:20,cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:500,color:statusFilter===s?"#7E9AFF":"#6B7280",padding:"5px 12px",transition:"all .15s"}}>{s}</button>
    ))}</>
  );
                      ...
                      <div style={{display:"flex",gap:14,flex:1}}>
                        ...
                        <div style={{flex:1}}>
                          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4,flexWrap:"wrap"}}>
                            ...
                            <span style={{background:sc.bg,color:sc.color}} className="tag">
                          ...
                          <div style={{color:"#9CA3AF",fontSize:13,marginBottom:8}}>
                            {app.neighborhood} · {app.type}{app.zoning?` · ${app.zoning}`:""} · {formatDate(app.submitted)}
                          </div>
            ...
            <div>
              ...
              <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:12}}>
                <div>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6,flexWrap:"wrap"}}>
                    ...
                    <span style={{background:sc.bg,color:sc.color}} className="tag"><span style={{width:5,height:5,borderRadius:"50%",background:sc.dot,display:"inline-block"}}></span>{selected.status}</span>
              ...
              {selected.source_url&&(
                <div style={{marginBottom:16}}>
                  <a href={selected.source_url} target="_blank" rel="noopener noreferrer" style={{color:"#7E9AFF",fontSize:13,display:"flex",alignItems:"center",gap:6}}>
                ...
                return(
                  <div key={a.id} className="card" style={{padding:"14px 18px",marginBottom:8,display:"flex",alignItems:"center",gap:16,cursor:"pointer"}} onClick={()=>openDetail(a)}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      ...
                      <span style={{background:sc.bg,color:sc.color}} className="tag"><span style={{width:5,height:5,borderRadius:"50%",background:sc.dot,display:"inline-block"}}></span>{a.status}</span>
                    </div>
                  </div>
                );
              })}
